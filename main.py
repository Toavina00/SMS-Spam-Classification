...
def main():
    ...
    X, y = tokenizer(df["text"].to_list(), return_tensors="pt", padding=True, truncation=True, max_length=512), torch.nn.functional.one_hot(torch.Tensor(df["label"]).long()).float()
